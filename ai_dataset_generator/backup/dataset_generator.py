...
class DatasetGenerator:
    ...
    def _generate_features(self):
        """Generate all feature variables as float."""
        for feature in self.dataset.features:
            name = feature['name']
            distribution = feature['distribution']
            
            # Generate raw values based on distribution (always float initially)
    ...
    def _apply_outliers(self):
        """Apply outlier injection to features (only non-categorical)."""
        for feature in self.dataset.features:
            outlier_rate = feature.get('outlier_rate', 0.0)
            
            # Skip if no outliers, or if categorical
            if outlier_rate == 0 or feature['name'] in self.categorical_features:
                continue
    ...
    def _convert_int_types(self):
        ...
        for feature in self.dataset.features:
            if feature['data_type'] == 'int' and feature['name'] not in self.categorical_features:
                name = feature['name']
    ...
    def _generate_target(self):
        ...
        # Only include non-categorical features in numeric form
        namespace = {'np': np}
        for feature_name, feature_values in self.data.items():
            ...
            # Categorical features are not added to namespace (can't use in expressions)
